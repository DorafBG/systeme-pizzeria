<title>PizzaLand - Paiement d'un panier</title>
...
<section class="recap-section">
        ...
        $lesProduits = Produit::getProduitsDePanier($_GET["IDPanier"]);
        $lesPizzas = PizzaDefaut::getPizzasDePanier($_GET["IDPanier"]);
        ...
        foreach ($lesProduits as $produit) {
            ...
            echo "<p>$nomProduit (x$quantiteProduit): $prixProduit €</p>";
        }
        ...
        foreach ($lesPizzas as $pizza) {
            ...
            echo "<p>$nomPizza: $prixPizza €</p>";
        }

        $prixtotal = Panier::getPrixTotalPanier($_GET["IDPanier"]);

        echo "</br>";
        echo "<strong><p>Total: $prixtotal €</p></strong>";
        ?>
    </section>
  
<div class="container" id="payment-container">

    <a class="exit-icon" href="?objet=panier"><svg  width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M13.7678 10.2322L10.2322 13.7678M13.7678 13.7678L10.2322 10.2322" stroke="#1B1B1B" stroke-width="1.5" stroke-linecap="round"/></svg></a>

<div class="heading">
<svg class="heading-icon" width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<form class="create-form" action="index.php" method="get">
        ...
        <input type="hidden" name="IDPanier" value="<?php echo $_GET['IDPanier']?>">
  <h1 class='infoPaiementTitre'>Informations de Paiement</h1>

  </div>
  
  <label for="name">Nom du Porteur</label>
    <input class="inputNomPorteur payment-input" type="text" id="name" name="NomPorteurCB" required placeholder="Asta Staria"/>
  <br>
  <label for="card">Numéro de carte</label>
  <input class="inputNumeroCarte payment-input" type="text" minlength="16" maxlength="16" id="card" name="codeCartePaiement" title='16 Chiffres de Carte Bleue' required placeholder="0000 0000 0000 0000" pattern="[0-9]{16}" />

  <br>
  <div class="exp-cvc">
    <div class="expiration">
  <label for="expiry">Date d'expiration</label>
  <input class="inputCard payment-input" name="datePeremptionPaiement" id="expiry" type="text" minlength="5" maxlength="5" pattern="(0[1-9]|1[0-2])\/[2-9][0-9]" title='Format MM/AA' required placeholder="00/00"/>
    <br>
    </div>
    <div class="security">
  <label for="cvc">CVV</label>
  <input class="inputCVV payment-input" type="text" minlength="3" maxlength="4" id="cvc" name="cryptogrammePaiement" title='Format XXX ou XXXX' required placeholder="XXX" pattern="[0-9]{3,4}" />
